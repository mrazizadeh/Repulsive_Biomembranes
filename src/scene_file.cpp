    ...
    namespace scene
    {
        ...
        std::string nameOfConstraint(ConstraintType type)
        {
            ...
            case ConstraintType::TotalArea:
                return "area";
            case ConstraintType::ReducedVolume:
                return "Reduced Volume";   
            case ConstraintType::ReducedMC:
                return "Reduced Mean Curvature";    
        ...
        GradientMethod methodOfName(std::string name)
        {
            ...
            else if (name == "helfrich")
            {
                return GradientMethod::Helfrich;
            }
            else if (name == "l2")
            {
        ...
        ConstraintType getConstraintType(std::string consType)
        {
            ...
            else if (consType == "reduced_volume")
            {
                return ConstraintType::ReducedVolume;
            }
            else if (consType == "reduced_mc")
            {
                return ConstraintType::ReducedMC;
            }
            else
            {
        ...
        void processLine(SceneData &data, std::string dir_root, std::vector<std::string> &parts)
        {
            ...
            else if (parts[0] == "minimize")
            {
                ...
                else if (parts[1] == "helfrich")
                {
                    data.potentials.push_back(PotentialData{PotentialType::Helfrich, weight, targetValue});
                    cout << "  * Adding Helfrich energy term (weight " << weight << ")" << endl;
                }
                else if (parts[1] == "helfrichADE")
                {
                    data.potentials.push_back(PotentialData{PotentialType::HelfrichADE, weight, targetValue});
                    cout << "  * Adding HelfrichADE energy term (weight " << weight << ")" << endl;
                }
                else if (parts[1] == "rv_soft")
                {
                    cout << "  trying ....RV " << weight << ")" << endl;
                    data.potentials.push_back(PotentialData{PotentialType::SoftReducedVolumeConstraint, weight, targetValue});
                    cout << "  * Adding RV energy term (weight " << weight << ")" << endl;

                }
                else if (parts[1] == "tmc_soft")

                {   
                    cout << "  trying ....tmc " << weight << ")" << endl;
                    data.potentials.push_back(PotentialData{PotentialType::SoftTotalMeanCurvatureConstraint, weight, targetValue});
                                        cout << "  * Adding TMC energy term (weight " << weight << ")" << endl;
