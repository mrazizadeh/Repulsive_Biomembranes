#include "energy/soft_tmc_constraint.h"
#include "matrix_utils.h"
#include "surface_derivatives.h"

namespace biorsurfaces
{
    SoftTotalMeanCurvatureConstraint::SoftTotalMeanCurvatureConstraint(MeshPtr mesh_, GeomPtr geom_, double weight_, double targetValue_)
    {
        mesh = mesh_;
        geom = geom_;
        weight = weight_;
...
for(GCHalfedge he : v.outgoingHalfedges()) {
  ...
  return at_xm;//SHOULD HAVE 1/3.0 but it does not work with FD computations
    ...
    double SoftTotalMeanCurvatureConstraint::Value()
    { 
        ...
        return weight*((m1/sqrt(4*geometrycentral::PI*m0))-targetValue)*((m1/sqrt(4*geometrycentral::PI*m0))-targetValue);
    }

    // Returns the current differential of the energy, stored in the given
    // V x 3 matrix, where each row holds the differential (a 3-vector) with
    // respect to the corresponding vertex.
    void SoftTotalMeanCurvatureConstraint::Differential(Eigen::MatrixXd &output)
    {
    ...
    // Initialize local variables for computation
        ...
        MatrixUtils::addToRow(output, inds[v_i], weight * dela_xm);
        }
    }

    // Get the exponents of this energy; only applies to tangent-point energies.
    Vector2 SoftTotalMeanCurvatureConstraint::GetExponents()
    {
        return Vector2{1, 0};
    }
    ...
    OptimizedClusterTree *SoftTotalMeanCurvatureConstraint::GetBVH()
    {
        return 0;
    }
    ...
    double SoftTotalMeanCurvatureConstraint::GetTheta()
    {
        return 0;
    }

    void SoftTotalMeanCurvatureConstraint::ResetTargetsTMC(double newTargetValue,double newWeight) {
